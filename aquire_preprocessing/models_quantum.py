...
class TorchStructuredReuploadingQuantumClassifier(nn.Module):
    """GPU-vectorized four-qubit circuit for waveform and clinical blocks.

    The first four coordinates enter through Y rotations. The second four
    enter through non-commuting Z and X rotations after the first variational
    and entangling layer. This preserves the two blocks as separate circuit
    operations while keeping the register at four qubits.
    """
    ...
    def __init__(self, topology: str = "ring"):
        ...
        n_qubits = 4
        if topology != "ring":
            raise ValueError("Structured circuit currently uses the frozen ring topology")
        edges = [(index, (index + 1) % n_qubits) for index in range(n_qubits)]
        self.n_qubits = n_qubits
        self.input_dim = 8
        self.edges = edges
        self.rotations = nn.Parameter(torch.empty(2, n_qubits, 3))
        self.interactions = nn.Parameter(torch.empty(2, len(edges)))
        self.feature_scales = nn.Parameter(torch.zeros(8))
        ...
        z_signs, zero_indices, one_indices = [], [], []
        ...
        self.register_buffer(
            "observable_signs",
            torch.stack(z_signs + [z_signs[left] * z_signs[right] for left, right in edges]),
        )
    ...
    def _apply_ry(self, state: torch.Tensor, angle: torch.Tensor, wire: int) -> torch.Tensor:
        zero, one = self.zero_indices[wire], self.one_indices[wire]
        ...
        result[:, zero] = cosine * left - sine * right
        result[:, one] = sine * left + cosine * right
        return result

    def _apply_rx(self, state: torch.Tensor, angle: torch.Tensor, wire: int) -> torch.Tensor:
        zero, one = self.zero_indices[wire], self.one_indices[wire]
        left, right = state.index_select(1, zero), state.index_select(1, one)
        cosine = torch.cos(angle / 2.0).to(state.dtype).reshape(-1, 1)
        sine = (-1j * torch.sin(angle / 2.0)).to(state.dtype).reshape(-1, 1)
        result = torch.empty_like(state)
        result[:, zero] = cosine * left + sine * right
        result[:, one] = sine * left + cosine * right
        return result
    ...
    def _apply_rz(self, state: torch.Tensor, angle: torch.Tensor, wire: int) -> torch.Tensor:
        ...
        result = torch.empty_like(state)
        result[:, zero] = torch.exp((-0.5j * angle).to(state.dtype)) * state.index_select(1, zero)
        result[:, one] = torch.exp((0.5j * angle).to(state.dtype)) * state.index_select(1, one)
        return result

    def quantum_observables(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 2 or x.shape[1] != 8 or not torch.isfinite(x).all():
            raise ValueError(f"Expected finite structured input (batch, 8), got {tuple(x.shape)}")
        state = torch.zeros(
            len(x), 1 << self.n_qubits,
            dtype=self._complex_dtype(x.dtype), device=x.device,
        )
        state[:, 0] = 1.0
        scaled = x * (2.0 * torch.sigmoid(self.feature_scales))
        for block in range(2):
            for wire in range(self.n_qubits):
                angle = scaled[:, block * self.n_qubits + wire]
                if block == 0:
                    state = self._apply_ry(state, angle, wire)
                else:
                    state = self._apply_rz(state, angle, wire)
                    state = self._apply_rx(state, 0.5 * angle, wire)
                state = self._apply_rz(state, self.rotations[block, wire, 0], wire)
                state = self._apply_ry(state, self.rotations[block, wire, 1], wire)
                state = self._apply_rz(state, self.rotations[block, wire, 2], wire)
            for edge_index in range(len(self.edges)):
                phase = torch.exp(
                    (-0.5j * self.interactions[block, edge_index] * self.edge_signs[edge_index]).to(state.dtype)
                )
                state = state * phase
