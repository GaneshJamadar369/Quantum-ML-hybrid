"""Private Kaggle QSVC screening job with frozen source revision."""
...
def main():
    ...
    run([sys.executable, "run_qsvc_hqnn_screen.py", "--task", "qsvc", "--representations", str(representations),
         "--metadata", str(metadata), "--output", "/kaggle/working/qsvc-screen",
         "--per-class", "500", "--seed", "20260922"], cwd=repo)
    print("QSVC screen complete; folds 9 and 10 sealed", flush=True)
