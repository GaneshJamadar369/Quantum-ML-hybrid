"""Pinned Kaggle GPU runner for the two-seed train-reference alignment screen."""
...
SOURCE_COMMIT = "e2b0dcc7807641380616704a67a176f764ef8f13"
SEEDS = (42, 31415)
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Score-alignment screen requires a Kaggle GPU")
    ...
    command(
        [sys.executable, "-m", "pytest", "-q", "tests/test_train_reference_score_alignment.py"],
        cwd=repo,
    )
    base = [
        sys.executable,
        ...
        "--output",
        "/kaggle/working/q4-score-alignment-screen-v1",
        "--seeds",
        *SEEDS,
        "--per-class",
        "2000",
