...
def _train_vqc(
    ...
    """Train an exact Torch-statevector VQC using Knowledge Distillation."""
    import torch
...
def run_independent_dual_route_screen(
    ...
    for held_out_value in sorted(np.unique(folds)):
        ...
        q_train_all, q_val, rep_audit = _fit_q4_representation(
            combined_h, combined_y, train_rows, val_rows, n_components=4,
            seed=seed + held_out,
        )
    ...
    for held_out_value in sorted(np.unique(folds)):
        ...
        q_train_all, q_val, _ = _fit_q4_representation(
            features_b_q, labels, train_idx, val_idx, n_components=4,
            seed=seed + held_out,
        )
        q_train = q_train_all[sample_idx]
    ...
    for held_out_value in sorted(np.unique(folds)):
        ...
        q_train_all, q_val, _ = _fit_q4_representation(
            features_b_c, labels, train_idx, val_idx, n_components=4,
